...
const Chat = () => {
  ...
  useEffect(() => {
    ...
    processCount("dispensing").then((data) => {
      ...
      setQuestionList([{ question: 'Display all dispensing projects we have done.', answer: JSON.stringify(data.allProcessDispensing) }]);
      setAiSql(`
    ${data.allProcessDispensingSql}
    `);
    setProcessCount(data.allProcessDispensing.map((item,index)=>({
      key:index,
