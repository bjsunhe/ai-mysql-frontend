...
const Chat = () => {
  ...
  useEffect(() => {
    ...
    processCount("dispensing").then((data) => {
      ...
      setQuestionList([{ question: 'How many projects are there for each process?', answer: JSON.stringify(data.processGroup) }]);
      setAiSql(`
    ${data.processGroupSql}
    `);
    setProcessCount(data.processGroup.map((item,index)=>({
      key:index,
      process:item._id,
      count:item.count
    })))
